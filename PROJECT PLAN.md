...
_inspired by GitHub Projects views_
...
- Date created: by inclusive date range.
  Desired date: - Presence (has date / no date). - Inclusive date range.
  Deadline: - Presence (has date / no date). - Inclusive date range.
...
Filters can be applied globally or within selected project(s). - If a user tries to delete a label used in any saved view, a warning will appear:
...
- Users can sort task lists by any of the following:
  ...
  - Section name (alphabetical) - If multiple sections across projects have the same name, their parent project will postfix the name for clarity in sorted views. - Example (sorted alphabetically by section): - In Progress – Personal - In Progress – Work - To Do – Work
...
- [ ]  A downloadable error log is shown for skipped entries. - [ ]   `csv
  Row,Error
3,Missing 'Task Title'
5,Invalid priority 'Urgent'
9,Deadline format not recognized (used DD-MM-YYYY)`
