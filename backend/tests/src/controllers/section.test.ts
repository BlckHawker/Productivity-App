...
describe("deleteSectionById", () => {
	...
	test("should return an error if the section doesn't exist", async () => {
		...
		const response = await sectionController.deleteSectionById(utils.prismaMock())(1);
	...
	test('should return an error if the section is named "Other"', async () => {
		...
		utils.mockCurried(sectionService.deleteSectionById as jest.Mock, otherSection);
		const response = await sectionController.deleteSectionById(utils.prismaMock())(1);
		expect(response).toBeInstanceOf(Error);
		expect((response as Error).message).toMatch(
	...
	test("should return an error if the service returns an error", async () => {
		utils.mockCurried(sectionService.getSectionById as jest.Mock, utils.mockedSection);
	...
	test("should return whatever deleteProjectById services returns", async () => {
		utils.mockCurried(sectionService.getSectionById as jest.Mock, utils.mockedSection);
		utils.mockCurried(sectionService.deleteSectionById as jest.Mock, utils.mockedSection);
		const response = await sectionController.deleteSectionById(utils.prismaMock())(1);
...
describe("createSection", () => {
	...
	test("Returns an error if the projectController returns an error", async () => {
		utils.mockCurried(projectController.getProjectById as jest.Mock, new Error());
		const response = await createSection();
		expect(response).toBeInstanceOf(Error);
	});

	test("Returns an error if a section with the same name exists in a project", async () => {
		utils.mockCurried(projectController.getProjectById as jest.Mock, utils.mockedProject);
	...
	test("Returns created section on success", async () => {
		utils.mockCurried(projectController.getProjectById as jest.Mock, utils.mockedProject);
		utils.mockCurried(sectionService.getSectionByName as jest.Mock, null);
		utils.mockCurried(sectionService.getNumberOfSectionInProject as jest.Mock, 0);
		utils.mockCurried(sectionService.createSection as jest.Mock, utils.mockedSection);
		const response = await createSection();
		expect(response).toBe(utils.mockedSection);
	});
...
describe("getAllSectionsInProject", () => {
	...
	test("Returns an array of all sections on success", async () => {
		...
		utils.mockCurried(sectionService.getAllSectionsInProject as jest.Mock, utils.mockedSection);
		const response = await getAllSectionsInProject();
		expect(response).toBe(utils.mockedSection);
	});
...
describe("getAllSections", () => {
	...
	test('Returns what section service\'s "getAllSections" provides', async () => {
		utils.mockFlat(sectionService.getAllSections as jest.Mock, utils.mockedSection);
...
describe("changeSectionName", () => {
	...
	test("If there is already a section with the new desired name, return an error", async () => {
		utils.mockCurried(sectionService.getSectionById as jest.Mock, utils.mockedSection);
		utils.mockCurried(projectController.getProjectById as jest.Mock, utils.mockedProject);
		utils.mockCurried(sectionService.getSectionByName as jest.Mock, utils.mockedSection);
		const response = await changeSectionName();
		expect(response).toBeInstanceOf(Error);
		expect((response as Error).message).toBe(`A section within the project named "${utils.mockedProject.name}" (id: ${utils.mockedProject.id}) already has a section named "${newName}". Cannot change the section named "${utils.mockedSection.name}" (id: ${id}) to "${newName}"`)
	})
	...
	test("If there are no conflicts, the name of the section should be changed and the update section object should be returned", async ()=> {
		...
		utils.mockCurried(projectController.getProjectById as jest.Mock, utils.mockedProject);
		utils.mockCurried(sectionService.getSectionByName as jest.Mock, null);
		utils.mockCurried(sectionService.changeSectionName as jest.Mock, utils.mockedSection);
		const response = await changeSectionName();
		expect(response).toBe(utils.mockedSection);		
	})
	...
	test("Returns an error if one was thrown", async () => {
		utils.mockCurried(sectionService.getSectionById as jest.Mock, utils.mockedSection);
		utils.mockCurried(projectController.getProjectById as jest.Mock, utils.mockedProject);
		utils.mockCurriedError(projectController.getProjectById as jest.Mock, new Error());
		const response = await changeSectionName();
		expect(response).toBeInstanceOf(Error);
	});
})
...
describe("moveSectionToProject", () => {
	...
	const moveSectionToProject = async (sectionId = section.id, newProjectId = new_project.id) =>
		await sectionController.moveSectionToProject(utils.prismaMock())(sectionId, newProjectId);
	...
	test("verify the new project is not the same as the one section is currently is", async () => {
		utils.mockCurried(sectionService.getSectionById as jest.Mock, section);
		utils.mockCurried(projectController.getProjectById as jest.Mock, old_project);
	...
	test("verify the project doesn't currently have the max amount of projects", async () => {
		...
		utils.mockCurried(sectionService.getSectionById as jest.Mock, section);
		utils.mockCurried(projectController.getProjectById as jest.Mock, new_project);
		utils.mockCurried(sectionService.getNumberOfSectionInProject as jest.Mock, sectionSize);
		const response = await moveSectionToProject();
		expect(response).toBeInstanceOf(Error);
	...
	test("move the section to the new project, and return the new section", async () => {
		...
		utils.mockCurried(sectionService.getNumberOfSectionInProject as jest.Mock, 0);
		utils.mockCurried(sectionService.getSectionByName as jest.Mock, null);
		utils.mockCurried(sectionService.changeSectionProject as jest.Mock, section);
		const response = await moveSectionToProject();
